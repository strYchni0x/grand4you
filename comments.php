...
if ( $comments ) : 
	...
	<div class="comments section-inner mw-thin max-percentage no-margin" id="comments">
		...
		if ( get_post_type() == 'product' ) {
			...
			$comments_title = sprintf( _nx( '%s Review', '%s Reviews', $comments_number, 'Translators: %s = the number of reviews', 'grand4you' ), $comments_number );
		} else {
			// Translators: %s = the number of comments.
			$comments_title = sprintf( _nx( '%s Comment', '%s Comments', $comments_number, 'Translators: %s = the number of comments', 'grand4you' ), $comments_number );
		}
		...
		$comment_pagination = paginate_comments_links( array(
			...
			'end_size'  => 0,
			'mid_size'  => 0,
			'next_text' => '<span class="text"><span class="long">' . esc_html__( 'Newer Comments', 'grand4you' ) . '</span><span class="short">' . esc_html__( 'Newer', 'grand4you' ) . '</span></span><span class="arrow">&rarr;</span>',
			'prev_text' => '<span class="arrow">&larr;</span><span class="text"><span class="long">' . esc_html__( 'Older Comments', 'grand4you' ) . '</span><span class="short">' . esc_html__( 'Older', 'grand4you' ) . '</span></span>',
		) );
