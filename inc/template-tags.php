	...
	function eksell_the_archive_filter() {
		...
		<div class="filter-wrapper i-a a-fade-up a-del-200">
			<ul class="filter-list reset-list-style">
				...
				<?php if ( $home_url ) : ?>
					<li class="filter-show-all"><a class="filter-link active" data-filter-post-type="<?php echo esc_attr( $post_type ); ?>" href="<?php echo esc_url( $home_url ); ?>"><span class="term-name"><?php esc_html_e( 'Show All', 'grand4you' ); ?></span></a></li>
				<?php endif; ?>
					...
					switch ( $post_meta_item ) {
						...
						case 'date' : 
							...
							<li class="date">
								<?php 
								if ( $context == 'single' ) {
									printf( esc_html_x( 'Published %s', '%s = The date of the post', 'grand4you' ), $entry_time_str );	
								} else {
									echo $entry_time_str;
								}
								?>
						...
						case 'author' : 
							...
							<li class="author">
								...
								printf( esc_html_x( 'By %s', '%s = author name', 'grand4you' ), '<a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author_meta( 'display_name' ) ) . '</a>' ); ?>
							</li>
							<?php
							break;
						...
						case 'categories' : 
							...
							$eksell_has_meta = true;
							$prefix = ( $context == 'single' ) ? esc_html__( 'Posted in', 'grand4you' ) : esc_html__( 'In', 'grand4you' );
							?>
							<li class="categories">
						...
						case 'tags' : 
							...
							<li class="tags">
								...
								if ( $tag_taxonomy == 'post_tag' ) {
									echo get_the_tag_list( esc_html__( 'Tagged', 'grand4you' ) . ' ', ', ', '', $post_id );
								} else {
									the_terms( $post_id, $tag_taxonomy, esc_html__( 'Tagged', 'grand4you' ) . ' ', ', ' ); 
								}
								?>
							</li>
							<?php
