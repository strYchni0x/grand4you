...
<form role="search" <?php echo $aria_label; ?> method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
	<label class="screen-reader-text" for="<?php echo esc_attr( $uniq_id ); ?>"><?php esc_html_e( 'Search For&hellip;', 'grand4you' ); ?></label>
	<input placeholder="<?php esc_attr_e( 'Search For&hellip;', 'grand4you' ); ?>" type="search" id="<?php echo esc_attr( $uniq_id ); ?>" class="search-field" value="<?php echo get_search_query(); ?>" name="s" />
	<button type="submit" class="search-submit reset stroke-cc">
		<span class="screen-reader-text"><?php echo esc_attr_x( 'Search', 'Submit button', 'grand4you' ); ?></span>
