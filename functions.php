...
if ( ! function_exists( 'eksell_theme_support' ) ) :
	function eksell_theme_support() {
		...
		load_theme_textdomain( 'grand4you', get_template_directory() . '/languages' );
		...
		add_theme_support( 'editor-font-sizes',
			array(
				array(
					'name'      => esc_html_x( 'Small', 'Name of the small font size in Gutenberg', 'grand4you' ),
					'shortName' => esc_html_x( 'S', 'Short name of the small font size in the Gutenberg editor.', 'grand4you' ),
					'size'      => 16,
					'slug'      => 'small',
				),
				array(
					'name'      => esc_html_x( 'Regular', 'Name of the regular font size in Gutenberg', 'grand4you' ),
					'shortName' => esc_html_x( 'M', 'Short name of the regular font size in the Gutenberg editor.', 'grand4you' ),
					'size'      => 18,
					'slug'      => 'normal',
				),
				array(
					'name'      => esc_html_x( 'Large', 'Name of the large font size in Gutenberg', 'grand4you' ),
					'shortName' => esc_html_x( 'L', 'Short name of the large font size in the Gutenberg editor.', 'grand4you' ),
					'size'      => 24,
					'slug'      => 'large',
				),
				array(
					'name'      => esc_html_x( 'Larger', 'Name of the larger font size in Gutenberg', 'grand4you' ),
					'shortName' => esc_html_x( 'XL', 'Short name of the larger font size in the Gutenberg editor.', 'grand4you' ),
					'size'      => 32,
					'slug'      => 'larger',
				)
			)
		...
		if ( $color_options_groups ) {
			...
			$editor_color_palette[] = array(
				'name'  => esc_html__( 'Background Color', 'grand4you' ),
...
if ( ! function_exists( 'eksell_register_nav_menus' ) ) :
	function eksell_register_nav_menus() {
		...
		register_nav_menus( array(
			'main'								=> esc_html__( 'Main Menu', 'grand4you' ),
			'footer'							=> esc_html__( 'Footer Menu', 'grand4you' ),
			$social_menu_args['theme_location']	=> esc_html__( 'Social Menu', 'grand4you' ),
		) );
	...
	function eksell_body_classes( $classes ) {
		...
		switch ( $pagination_type ) {
			...
			case 'links':
				...
				break;
			case 'numbers':
				$classes[] = 'pagination-type-numbers';
				break;
		}
...
/*	-----------------------------------------------------------------------------------------------
	POSTS PER PAGE (NUMBERED PAGINATION)
	When the "Numbered pages" pagination type is active, override posts_per_page on the main
	query for archive / home pages using the value stored in the Customizer.
--------------------------------------------------------------------------------------------------- */

if ( ! function_exists( 'eksell_set_posts_per_page' ) ) :
	function eksell_set_posts_per_page( $query ) {

		// Only touch the main query, only on the front-end, only on archive-like pages.
		...
		// Only apply when the numbered pagination type is selected.
		if ( 'numbers' !== get_theme_mod( 'eksell_pagination_type', 'button' ) ) {
			return;
		}

		$posts_per_page = absint( get_theme_mod( 'eksell_posts_per_page', get_option( 'posts_per_page', 10 ) ) );
	...
	add_action( 'pre_get_posts', 'eksell_set_posts_per_page' );
endif;
...
if ( ! function_exists( 'eksell_get_the_archive_title_prefix' ) ) :
	function eksell_get_the_archive_title_prefix() {
		...
		if ( is_search() ) {
			$prefix = esc_html_x( 'Search Results', 'search archive title prefix', 'grand4you' );
		} elseif ( is_category() ) {
			$prefix = esc_html_x( 'Category', 'category archive title prefix', 'grand4you' );
		} elseif ( is_tag() ) {
			$prefix = esc_html_x( 'Tag', 'tag archive title prefix', 'grand4you' );
		} elseif ( is_author() ) {
			$prefix = esc_html_x( 'Author', 'author archive title prefix', 'grand4you' );
		} elseif ( is_year() ) {
			$prefix = esc_html_x( 'Year', 'date archive title prefix', 'grand4you' );
		} elseif ( is_month() ) {
			$prefix = esc_html_x( 'Month', 'date archive title prefix', 'grand4you' );
		} elseif ( is_day() ) {
			$prefix = esc_html_x( 'Day', 'date archive title prefix', 'grand4you' );
			...
			if ( $queried_object ) {
				...
				$prefix = sprintf(
					...
					esc_html_x( '%s:', 'taxonomy term archive title prefix', 'grand4you' ),
					$tax->labels->singular_name
				);
			}
		} elseif ( is_home() && is_paged() ) {
			$prefix = esc_html_x( 'Archives', 'general archive title prefix', 'grand4you' );
		}
...
if ( ! function_exists( 'eksell_filter_archive_title' ) ) :
	function eksell_filter_archive_title( $title ) {
		...
		elseif ( is_home() && is_paged() ) {
			...
			$title 	= sprintf( esc_html_x( 'Page %1$s of %2$s', '%1$s = Current page number, %2$s = Number of pages', 'grand4you' ), $paged, $max );
		}
	...
	function eksell_filter_archive_description( $description ) {
		...
		elseif ( is_search() ) {
			...
			if ( $wp_query->found_posts ) {
				...
				$description = esc_html( sprintf( _nx( 'We found %s result for your search.', 'We found %s results for your search.',  $wp_query->found_posts, '%s = Number of results', 'grand4you' ), $wp_query->found_posts ) );
			} else {
				$description = esc_html__( 'We could not find any results for your search. You can give it another try through the search form below.', 'grand4you' );
			}
		}
	...
	function eksell_filter_nav_menu_item_args( $args, $item, $depth ) {
		...
		if ( $args->theme_location == 'main' && isset( $args->show_toggles ) ) {
			...
			if ( in_array( 'menu-item-has-children', $item->classes ) ) {
				...
				$args->after .= '<div class="sub-menu-toggle-wrapper"><a href="#" class="toggle sub-menu-toggle stroke-cc" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="250"><span class="screen-reader-text">' . esc_html__( 'Show sub menu', 'grand4you' ) . '</span>' . eksell_get_theme_svg( 'ui', 'chevron-down', 18, 10 ) . '</a></div>';
...
if ( ! function_exists( 'eksell_filter_comment_text' ) ) :
	function eksell_filter_comment_text( $comment_text, $comment, $args ) {
		...
		if ( eksell_is_comment_by_post_author( $comment ) ) {
			$comment_text .= '<p class="by-post-author">' . esc_html__( 'By Post Author', 'grand4you' ) . '</p>';
		}
